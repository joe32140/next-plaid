...
//!   D. int8 × int8       — both sides int8 in memory (4× storage, not 32×),
//!                          hardware SDOT. The "store binary as int8" upper end.
//!
//! Run:
...
fn main() {
    println!(
        "int8 × 1-bit kernel microbenchmark\n\
         ...
         (storage fixed at packed bits for A/C and the fused kernels; D stores int8)\n"
    );
    ...
    let q_int8 = quantize_i8(&query.view());
    ...
    let (t_d, _) = best_of(|| docs_i8.iter().map(|d| maxsim_i8(&q_int8, d, DIM)).sum());
    ...
    println!(
        "  D. int8 x int8 SDOT    ( 4x):  {:>7.3} us/doc   {:.2}x",
        us(t_d),
        rel(t_d)
    );
